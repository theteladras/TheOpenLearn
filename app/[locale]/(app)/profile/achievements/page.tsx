...
import { AchievementGroupShell } from "@/features/achievements/achievement-group-shell";
import { humanizeSkillKeyForAchievement } from "@/lib/achievement-humanize";
import {
  type AchievementGroupKind,
  buildAchievementGroups,
  countEarnedInGroup,
} from "@/lib/achievement-groups";
...
export default async function ProfileAchievementsPage({ params }: Props) {
  ...
  const groups = buildAchievementGroups(sorted);
  ...
  function groupTitle(
    ...
    if (kind === "core") return t("groupCoreTitle");
    if (kind === "cluster" && subgroupKey)
      return t("groupClusterSection", { cluster: tCluster(subgroupKey) });
    if (kind === "skill" && subgroupKey) {
      const skillDisplay =
        isTaskAchievementKey(subgroupKey) ?
          tSkillKey(subgroupKey)
        : humanizeSkillKeyForAchievement(subgroupKey);
      return t("groupSkillSection", { skill: skillDisplay });
    }
    return t("groupOtherTitle");
  }
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="space-y-3">
        {groups.map((g) => {
          const { earned: ge, total: gt } = countEarnedInGroup(
            g.items,
            earnedSet,
          );
          return (
            <AchievementGroupShell
              key={g.id}
              title={groupTitle(g.kind, g.subgroupKey)}
              earned={ge}
              total={gt}
            >
              <ul className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                {g.items.map((a) => {
                  ...
                  const clusterLabel = clusterParsed ?
                    tCluster(clusterParsed.key)
                  : "";
                  const skillDisplay =
                    skillParsed ?
                  ...
                  const description =
                    ...
                    : clusterParsed ?
                      tAch(
                        `clusterMilestone.${clusterParsed.tier}.description`,
                        {
                          cluster: clusterLabel,
                        },
                      )
                  ...
                  if (skillParsed) {
                    ...
                    howToBody = tAch(
                      `howTo.clusterMilestone.${clusterParsed.tier}`,
                      {
                        cluster: clusterLabel,
                      },
                    );
